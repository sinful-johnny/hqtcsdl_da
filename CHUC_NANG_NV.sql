...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM V_LICHLAMVIEC_NHANVIEN WHERE NGAYKHAM = @NGAYKHAM AND GIOKHAM = @GIOKHAM AND TRANGTHAI LIKE N'Trống')
    BEGIN
        UPDATE LICH_LAM_VIEC
        SET TRANGTHAI = N'Đã đặt'
        WHERE NGAYKHAM = @NGAYKHAM AND GIOKHAM = @GIOKHAM AND TRANGTHAI LIKE N'Trống'

        SELECT @ID_LLV = ID_LLV
        FROM V_LICHLAMVIEC_NHANVIEN
        WHERE NGAYKHAM = @NGAYKHAM AND GIOKHAM = @GIOKHAM AND TRANGTHAI = N'Đã đặt'

        INSERT INTO V_LICHDATKHAM_NHANVIEN (ID_LLV, ID_KH, ID_NV)
    ...
    BEGIN
        ROLLBACK;
        PRINT 'Hiện không làm việc vào thời gian này!'
    END
END
GO
